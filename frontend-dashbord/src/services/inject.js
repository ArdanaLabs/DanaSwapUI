const extensionId = "poonlenmfdfbjfeeballhiibknlknepo";
...
window.cardano1 = Object.freeze(new CardanoAPI())
`
...
function createYoroiPort() {
    // events from Yoroi
    yoroiPort = window.chrome.runtime.connect(extensionId);
...
if (shouldInject()) {
    console.log(`content script injected into ${window.location.hostname}`);
    ...
    window.addEventListener("message", function(event) {
        ...
        if (dataType === "connector_rpc_request") {
            ...
            if ((ergoApiInjected || cardanoApiInjected) && yoroiPort) {
                ...
                // convertImgToBase64(getFavicon(window.location.origin))
                //     .then(imgBase64Url => {
